...
def save_mapping(item_id, board_id, sf_id, entity_type):
    ...
    VALUES (%s, %s, %s, %s)
    ON CONFLICT (item_id) DO UPDATE SET sf_id = EXCLUDED.sf_id;
...
def get_sf_id(item_id, board_id):
    conn = psycopg2.connect(**DB_CONFIG)
    cur = conn.cursor()
    cur.execute("SELECT sf_id FROM item_sf_mapping WHERE item_id = %s AND board_id = %s;", (item_id, board_id))
    row = cur.fetchone()
    cur.close()
    conn.close()
...
def delete_mapping(item_id):
    conn = psycopg2.connect(**DB_CONFIG)
    cur = conn.cursor()
    cur.execute("DELETE FROM item_sf_mapping WHERE item_id = %s;", (item_id,))
    conn.commit()
    cur.close()
    conn.close()
